...
public class BSYTextViewVertical extends View {
    ...
    public BSYTextViewVertical(Context context) {
        super(context);
        init();
    }

    public BSYTextViewVertical(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BSYTextViewVertical);
        int count = typedArray.getIndexCount();
        for (int i = 0; i < count; i++) {
            int index = typedArray.getIndex(i);
            if (index == R.styleable.BSYTextViewVertical_v_start) {
                startOrientation = typedArray.getInt(index, START_LEFT);
            } else if (index == R.styleable.BSYTextViewVertical_v_text) {
                text = typedArray.getString(index);
            } else if (index == R.styleable.BSYTextViewVertical_v_textColor) {
                textColor = typedArray.getColor(index, Color.BLACK);
            } else if (index == R.styleable.BSYTextViewVertical_v_textSize) {
                textSize = typedArray.getDimension(index, 16);
            } else if (index == R.styleable.BSYTextViewVertical_v_cutChars) {
                cutChars = typedArray.getString(index);
            } else if (index == R.styleable.BSYTextViewVertical_v_textVerticalMargin) {
                textVerticalMargin = typedArray.getDimension(index, textVerticalMargin);
            } else if (index == R.styleable.BSYTextViewVertical_v_textHorizontalMargin) {
                textHorizontalMargin = typedArray.getDimension(index, textHorizontalMargin);
            } else if (index == R.styleable.BSYTextViewVertical_v_line) {
                lineOrientation = typedArray.getInt(index, NONE);
            } else if (index == R.styleable.BSYTextViewVertical_v_lineWidth) {
                lineWidth = typedArray.getDimension(index, lineWidth);
            } else if (index == R.styleable.BSYTextViewVertical_v_lineColor) {
                lineColor = typedArray.getColor(index, Color.BLACK);
            } else if (index == R.styleable.BSYTextViewVertical_v_line2TextMargin) {
